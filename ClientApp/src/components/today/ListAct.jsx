...
export default function ListTodo(props){
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            ...
            const check = (x) =>  today.getFullYear() === new Date(Date.parse(x.date)).getFullYear() &&
            today.getMonth() === new Date(Date.parse(x.date)).getMonth() &&
            today.getDate() === new Date(Date.parse(x.date)).getDate();
            let x=[];
